...
public class ProjectFollowController {
    ...
    public ResponseEntity<List<UserDTO.UserSimpleInfo>> getProjectFollowers(@PathVariable("projectId") Long projectId) {
        List<UserDTO.UserSimpleInfo> response = projectFollowService.getProjectFollowers(projectId);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @ApiOperation(value = "유저가 팔로우하는 프로젝트 조회")
    @GetMapping("/users/{userId}/project-follow")
    public ResponseEntity<List<ProjectDTO.ProjectListResponse>> getFollowingProjectsByUser(@PathVariable("userId") String userId) {
        List<ProjectDTO.ProjectListResponse> response = projectFollowService.getFollowingProjectsByUser(userId);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
}
